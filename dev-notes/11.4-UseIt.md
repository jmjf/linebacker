...
-  Message handler is broken because `StoreStatusReceived` event needs `retryCount`
-  Prisma `initModule` and `expressApp` are broken because they needs a circuit breaker for the queue adapter
   -  Need to add queue circuit breaker to Prisma circuit breaker build
