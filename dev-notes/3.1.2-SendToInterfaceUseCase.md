...
[] Assemble message DTO for backup store interface
[] Send message to backup store interface (adapter call, so need an adapter)
[] Set request status, status timestamp, etc.
[] Update request in repo

## Add methods to backup request entity
[] Set status to sent